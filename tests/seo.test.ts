...
test("Spanish SEO URLs are self-canonical and point back to English", () => {
  ...
  assert.equal(new URL(urls.languages["en-US"]).search, "?jurisdiction=san-mateo");
  assert.equal(
...
test("English SEO URLs are self-canonical on the unsuffixed URL", () => {
  const urls = localizedSeoUrls("/about?lang=en", "en");

  assert.equal(new URL(urls.canonical).pathname, "/about");
  // Canonical must be the URL a crawler can fetch and get this same page from,
  // not the ?lang=en duplicate of it.
  assert.equal(new URL(urls.canonical).search, "");
  assert.equal(new URL(urls.languages["en-US"]).search, "");
